...
module.exports = (
	...
	let tableRows = parserHtml(html).querySelectorAll(
		'table.b_items_list tbody tr'
	);
	...
	let data = tableRows.reduce((res, el, idx) => {
		...
		const { title, htmlBody, seoTitle, seoKeywords } = workingWithName(
			name,
			catalogTitle
		);
		...
		return (
			res +
			`${amirogen};${idTitle}${idx +
				addToIdx};${title};${htmlBody};${price};${imgLink};${imgLink};${imgLink};${sku};${seoTitle};${seoKeywords};${title};false;Китай;На складе\n`
		);
	}, '');
...
function getInfo(param, node) {
	switch (param) {
		case 'sku':
			return getNodeInner(node, 'span[itemprop="sku"]');

		case 'price':
			return priceCalculation(getNodeInner(node, 'td.bil_price'));

		case 'name':
			return getNodeInner(node, 'span[itemprop="name"]');
	}
}
